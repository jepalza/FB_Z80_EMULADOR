...
Sub daa_a()
    ...
    If (fH = cierto) Or ((regA And &hF) > &h9) Then
        incr = incr Or &h6
    End If
    
    If (fC = cierto) Or (regA > &h9F) Then
        incr = incr Or &h60
    End If
...
End Function
 ...
 Function getF() As Integer
    ...
    ' // the boolean Datos into 0/1 rather than the slower
    ' // "Abs(fS)" method
    getF = (-(fS)  * F_S)  Or _
           (-(fZ)  * F_Z)  Or _
           (-(f5)  * F_5)  Or _
...
Sub execute()
    ...
    Dim xxx As Integer
    Dim TStates As Integer=0 ' estados a descontar a ResTStates en cada OP ejecutada	  	
    Dim opcode As Integer
    Dim oldTick as uInteger
    
    ' para calcular la velocidad real del Z80 frente a un PC
    oldTick = Timer * 10000
    
    ' bucle "nunca-sale" (solo pulsando 'ESC')
    While cierto
  
    ResTStates += TStates ' descontamos estados "gastados" en cada OP ejecutada
 
    ' contador de refresco de pantalla (240 lineas)
    If (linenum < 240) Then drawLine (linenum) : linenum+=1 :vsync=0
        
    ' IRQ
    If (ResTStates >= 0) Then	
    	  ' miramos su vamos "muy rapido" en el PC en el que se ejecuta
    	  ' quitar para ir en modo TURBO
        ltemp = ((Timer*10000) - oldTick)
        If (ltemp <6) Then Sleep 6-ltemp,1 ' lo frenamos
        oldTick = Timer*10000
            
        ' se reinicializa el contador
        ResTStates = -TstatesPerInterrupt - interrupt()
     
        ' solo refrescamos la pantalla fuera de barrido              
        If (linenum = 240) Then
            ponpantalla
            linenum+=1
        End If
        If linenum>240 And linenum<275 Then
        	   linenum += 1
        	   vsync=1
        EndIf
        If linenum=275 Then linenum=0
        LeeTeclado
    End If
    ...
    RefreshIRQ(1)
       ...
       veces_a_pintar2+=1
       if deb And veces_a_pintar2 >1000 Then 
         If MultiKey(SC_F12) then BreakPoint=1
         If BreakPoint=1 then sleep
         veces_a_pintar2=0
         locate 15,1
         print "PC="+hex$(regpc,4)+"  Byte:"+hex$(peekb(regpc),4)+"  Word+1:"+hex$(peekw(regpc+1),4)
         print "SP="+hex$(regsp,4)
         Print
         print "A ="+hex$(regA,2)
         print "B ="+hex$(regB,2)
         print "C ="+hex$(regC,2)
         print "DE="+hex$(regDE,4)
         print "HL="+hex$(regHL,4)
         Print
         print "AF'="+hex$(regAF_,4),"IX="+hex$(regIX,4)
         print "BC'="+hex$(regBC_,4),"IY="+hex$(regIY,4)
         print "DE'="+hex$(regDE_,4),"ID="+hex$(regID,4)
         print "HL'="+hex$(regHL_,4)
         
         
	Locate 1,50:Print Hex(tabpeek(0)),Hex(tabpoke(0))
	Locate 2,50:Print Hex(tabpeek(1)),Hex(tabpoke(1))
	Locate 3,50:Print Hex(tabpeek(2)),Hex(tabpoke(2))
	Locate 4,50:Print Hex(tabpeek(3)),Hex(tabpoke(3))
	Locate 16,1
        ...
        ' comienza la ejecucion de instrucciones
        xxx = peekb(regPC)
        regPC = regPC + 1
...
Function interrupt() As Integer
    ...
    If intIFF1 = Falso Then
        interrupt = 0 ' contador de ciclos de ejecucion
    Else
    	  CntHSync and= &h1F ' contador de sinconismos del CPC
        Select Case intIM
        Case 0, 1
            pushpc
            intIFF1 = Falso
            intIFF2 = Falso
            regPC = 56 ' 0x38
            interrupt = 11 '13 ' contador de ciclos de ejecucion
        	Case 2
            pushpc
            intIFF1 = Falso
            intIFF2 = Falso
            regPC = peekw((intI * 256) Or &hFF)
            interrupt = 17 '19 ' contador de ciclos de ejecucion
        End Select
    End If
