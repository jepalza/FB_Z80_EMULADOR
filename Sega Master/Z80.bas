...
Sub execute()
    ...
    Dim xxx As integer, TStates As Integer
        	  	
    Dim opcode As integer, oldTick as Integer
    ...
    If irqsetLine Then
       interrupt() ' tratamientos de IRQ de Z80
    End If
    ...
    If (ResTStates >= 0) Then
        ...
        ResTStates = ResTStates - TstatesPerInterrupt  '// Reset Interrupt Counter.
        vdp_interrupt (linenum)                        '// Handle VDP Interrupt.
        If (linenum < 192) Then drawLine (linenum)     '// Draw Next VDP Scanline.
        ...
        Else
            ...
            '--------------------------------------+
            ltemp = (Timer - oldTick)
            If (ltemp < 15) Then Sleep (15 - ltemp),1
            oldTick = timer
            '-------------------------------------+
            '        1000 \ (50) = 20 (PAL).      |
            '-------------------------------------+
...
execute_end:
        ...
        ResTStates = ResTStates + TStates ' descontamos estados "gastados" en cada OP ejecutada
