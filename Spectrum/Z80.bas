' Emulador Z80 retocado para que funcione en FreeBasic
...
Declare Function interrupt() As long
Declare Sub      execute()
Declare Sub      Z80Reset()

' Puertos entrada/salida (a escribir para cada emulacion)
Declare Function inb(port As Long) As Long
Declare Sub outb(port As Long, outbyte As Long)
...
dim shared BreakPoint as long

' instrucciones
Declare Sub      adc_a(b As long)
Declare Function adc16(a As long, b As long) As long
Declare Sub      add_a(b As long)
Declare Function add16(a As long, b As long) As long
Declare Sub      and_a(b As long)
Declare Sub      ccf()
Declare Sub      cp_a(b As long)
Declare Sub      cpl_a()
Declare Sub      daa_a()
Declare Function dec16(a As long) As long
Declare Sub      ex_af_af()
Declare Sub      exx()
Declare Function getAF() As long
Declare Function getBC() As long
Declare Function getD() As long
Declare Function getE() As long
Declare Function getF() As long
Declare Function getH() As long
Declare Function getIDH() As long
Declare Function getIDL() As long
Declare Function getL() As long
Declare Function id_d() As long
Declare Sub      ld_a_i()
Declare Sub      ld_a_r()
Declare Sub      neg_a()
Declare Sub      rld_a()
Declare Sub      setIDH(byteval As long)
Declare Sub      setIDL(byteval As long)
Declare Function in_bc() As long
Declare Function inc8(ByVal ans As long) As long
Declare Function dec8(ByVal ans As long) As long
Declare Sub      or_a(b As long)
Declare Sub      poppc()
Declare Function popw() As long
Declare Sub      pushpc()
Declare Sub      pushw(word As long)
Declare Function rl(ans As long) As long
Declare Sub      rl_a()
Declare Function rlc(ans As long) As long
Declare Sub      rlc_a()
Declare Function rr(ans As long) As long
Declare Sub      rr_a()
Declare Function rrc(ans As long) As long
Declare Sub      rrd_a()
Declare Sub      rrc_a()
Declare Sub      sbc_a(b As long)
Declare Function sbc16(a As long, b As long) As long
Declare Sub      scf()
Declare Sub      setAF(v As long)
Declare Sub      setBC(nn As long)
Declare Function qdec8(a As long) As long
Declare Function inc16(a As long) As long
Declare Sub      setD(l As long)
Declare Sub      setE(l As long)
Declare Sub      setF(b As long)
Declare Sub      setH(l As long)
Declare Sub      setL(l As long)
Declare Function sla(ByVal ans As long) As long
Declare Function sls(ByVal ans As long) As long
Declare Function sra(ByVal ans As long) As long
Declare Function srl(ByVal ans As long) As long
Declare Sub      sub_a(b As long)
Declare Sub      xor_a(b As long)

' llamadas a memoria
Declare Sub      pokeb(addr As long, newByte As long)
Declare Sub      pokew(addr As long, word As long)
'Declare Sub      pokeb_NoContendedDelay(addr As long, newByte As long)

Declare Function peekb(addr As long) As long
Declare Function peekw(addr As long) As long
'Declare Function peekb_NoContendedDelay(addr As long) As long


' varias
Declare Sub      bitv(b As long, r As long)
Declare Function bitResv(bitvalor As long, valor As long) As long
Declare Function bitSetv(bitvalor As long, valor As long) As long
Declare Function execute_cb() As long
Declare Function execute_ed() As long
Declare Sub      execute_id_cb(op As long, ByVal z As long)
Declare Sub      REFRESH(t As long)
Declare Function execute_id() As long
Declare Function nxtpcw() As long
Declare Function nxtpcb() As long

' *******************************
'  variables empleadas en el Z80
' *******************************

Dim Shared Parity(256) As Long
Dim Shared glContendedMemoryDelay As Long  ' // Contains number of tstates added due to memory contention for the current opcode


' tabla de divisiones, mas rapido que dividir en tiempo real 
Dim Shared Div4    (82000) As Long
Dim Shared Div32   (82000) As Long
Dim Shared Div256  (82000) As Long 
Dim Shared Div16384(82000) As Long 


' // Main Z80 registers //
Dim Shared regA As Long
Dim Shared regHL As Long
Dim Shared regB As Long
Dim Shared regC As Long
Dim Shared regDE As Long

' // Z80 Flags
Dim Shared fS As Long
Dim Shared fZ As Long
Dim Shared f5 As Long
Dim Shared fH As Long
Dim Shared f3 As Long
Dim Shared fPV As Long
Dim Shared fN As Long
Dim Shared fC As Long

' // Flag bit positions
Const F_C As Long = 1
Const F_N As Long = 2
Const F_PV As Long = 4
Const F_3 As Long = 8
Const F_H As Long = 16
Const F_5 As Long = 32
Const F_Z As Long = 64
Const F_S As Long = 128

' // Alternate registers //
Dim Shared regAF_ As Long
Dim Shared regHL_ As Long
Dim Shared regBC_ As Long
Dim Shared regDE_ As Long

' // Index registers  - ID used as temp for ix/iy
Dim Shared regIX As Long
Dim Shared regIY As Long
Dim Shared regID As Long

' // Stack pointer and program counter
Dim Shared regSP As Long
Dim Shared regPC As Long

' // Interrupt registers and flip-flops, and refresh registers
Dim Shared intI As Long
Dim Shared intR As Long
Dim Shared intRTemp As Long
Dim Shared intIFF1 As Long
Dim Shared intIFF2 As Long
Dim Shared intIM As Long
...
Function adc16(a As long, b As long) As long
...
Function add16(a As long, b As long) As long
    Dim ans As long
    ans = (a + b) And &HFFFF&
    f3 = (ans And (F_3 * 256&)) <> 0&
    f5 = (ans And (F_5 * 256&)) <> 0&
...
Sub bitv(b As long, r As long)
    Dim IsbitSet As long
    IsbitSet = (r And b) <> 0&
    fN = False
    fH = True
    'f3 = (r And F_3) <> 0& ' estaban anulados, no se porque
    'f5 = (r And F_5) <> 0& ' los habilite a ver si hay diferencias
    fS = IsbitSet And (b = F_S)
    fZ = (IsbitSet = 0)
    fPV = fZ
End Sub

Function bitResv(bitvalor As long, valor As long) As long
    bitResv = valor And (bitvalor Xor &HFFFF&)
End Function

Function bitSetv(bitvalor As long, valor As long) As long
...
Function dec16(a As long) As long
    dec16 = (a - 1&) And &HFFFF&
End Function
...
ex_ed160_191:
    ...
    Case 187 ' OTDR
        ...
        End If
    Case 187 To 191
        Print "Instruccion ED desconocida " ; xxx ; " en " ; regPC
        execute_ed = 8
...
Sub execute_id_cb(op As long, ByVal z As long)
...
End Sub
 ...
 Function getAF() As long
    getAF = (regA * 256&) Or getF
End Function

Function getBC() As long
    getBC = (regB * 256&) Or regC
End Function

Function getD() As long
    getD = Div256(regDE)
End Function
Function getE() As long
    getE = regDE And &HFF&
End Function

 Function getF() As long
...
Function getH() As long
    getH = Div256(regHL)
End Function

Function getIDH() As long
    getIDH = Div256(regID)
End Function
Function getIDL() As long
    getIDL = regID And &HFF&
End Function


Function getL() As long
    getL = regHL And &HFF&
End Function

Function id_d() As long
    Dim d As long
...
Function inc8(ByVal ans As long) As long
    fPV = (ans = &H7F&)
...
Function dec8(ByVal ans As long) As long
    fPV = (ans = &H80&)
    fH = (((ans And &HF&) - 1) And F_H) <> 0
...
Sub REFRESH(t As long)
    intRTemp = intRTemp + t
End Sub

Function rl(ans As long) As long
    Dim c As long
    Dim rl_Aux as long
...
Function rlc(ans As long) As long
    Dim rlc_Aux as long
...
Function rr(ans As long) As long
    Dim c As long
    Dim rr_Aux as long
...
Function rrc(ans As long) As long
    Dim rrc_Aux as long
...
Function sbc16(a As long, b As long) As long
    Dim c As long, ans As long
...
'IF REGPC=34423 THEN DEB=2:BreakPoint=1
       if deb then 
...
Function qdec8(a As long) As long
    qdec8 = (a - 1) And &HFF&
End Function

Function execute_id() As long
...
ex_id112_119:
    ...
    Case 117 ' LD (ID+d),L
        ...
        execute_id = 19
    Case 118 ' UNKNOWN
        Print "Instruccion desconocida " ; xxx ; " en " ; regPC
    Case 119 ' LD (ID+d),A
        pokeb id_d, regA
        execute_id = 19
    End Select
...
Function inc16(a As long) As long
    inc16 = (a + 1&) And &HFFFF&
End Function
...
Function peekb(addr As long) As long
    addr=addr and &hffff
...
Function peekw(addr As long) As long
...
Sub pokeb(addr As long, newByte As long)
    addr=addr and &hffff
...
Function inb(port As long) as long
    ...
    If (port And &HFF&) = 254& Then
        ...
        If (port And &H8000&) = 0& Then res = res And keyB_SPC
        If (port And &H4000&) = 0& Then res = res And keyH_ENT
        If (port And &H2000&) = 0& Then res = res And keyY_P
        If (port And &H1000&) = 0& Then res = res And key6_0
        If (port And &H800&) = 0& Then res = res And key1_5
        If (port And &H400&) = 0& Then res = res And keyQ_T
        If (port And &H200&) = 0& Then res = res And keyA_G
        If (port And &H100&) = 0& Then res = res And keyCAPS_V
 
        inb = (res And glKeyPortMask) ' anulado hasta debugar: Or glEarBit ' glEarBit holds tape state (0 or 64 only)
        glTStates = glTStates + glContentionTable(-glTStates)
...
Sub outb(port As long, outbyte As long)
    If (port And 1&) = 0& Then
        ...
        glNewBorder = outbyte And &H7& 'glNormalColor(outbyte And &H7&)
...
'        If glBufNum = NUM_WAV_BUFFERS Then glBufNum = 0
   ...
   ' miramos el tiempo transcurrido
    lSleep = (Timer()*1000) - glInterruptTimer
